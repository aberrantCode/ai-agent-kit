...
> **Bot / agent framework detected?** If the codebase is built around an agent loop, tool registry, session manager, and message bus, wire Honcho in at two seams: (1) expose `peer.chat(query)` as a **tool** in the registry so the agent can query user context on demand (see *Pattern A* below), and (2) hook the message bus so every inbound/outbound turn is persisted via `session.add_messages(...)`. Map one Honcho session per conversation thread and create a peer per participant (human *and* bot).
