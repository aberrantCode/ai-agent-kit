...
> **`<skill-dir>`** is this bundle's own directory — `~/.claude/skills/session-handoff/sub-skills/handoff`
> when installed to the global profile (this sub-skill lives nested under the `session-handoff`
> umbrella skill, so every `<skill-dir>/scripts/…` reference below resolves through that extra
> `sub-skills/handoff` segment). Every `<skill-dir>/scripts/…` reference below — **including
