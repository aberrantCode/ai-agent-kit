...
- **`AGENTS.md`** — If absent, copy from `references/init-project/AGENTS.md.template` with `{{PROJECT_NAME}}` substituted. If present, check whether it already contains the marker `<!-- BEGIN project-manager fragment -->`. If no, **append** the contents of `CLAUDE_FRAGMENT.md.template` (yes, named for CLAUDE but the same fragment is valid for AGENTS); never replace existing content. If yes, apply the **fragment-version self-heal** described for `CLAUDE.md` below (compare the embedded `pm-fragment-version` against the template; on a stale version write `AGENTS.md.new` and surface via `AskUserQuestion`, never overwrite in place).
- **`CLAUDE.md`** — Same as AGENTS.md, plus **fragment-version self-heal**. If the `<!-- BEGIN project-manager fragment -->` marker is absent, append the fragment. If present, read the `<!-- pm-fragment-version: N -->` line inside the existing `BEGIN…END` block (a missing line counts as version `1`) and compare it to the `pm-fragment-version` in `CLAUDE_FRAGMENT.md.template`. If the existing version is **lower** (or the delimited block otherwise differs from the current template), the fragment is **stale** — do NOT no-op: write a refreshed copy of the whole file, with only the `BEGIN…END` region replaced by the current fragment, to `CLAUDE.md.new`, then surface the drift via `AskUserQuestion` so the user can merge. Never overwrite `CLAUDE.md` in place. If the versions match, no-op.
...
- The CLAUDE.md / AGENTS.md fragment is detected by marker comments and not appended twice; when the marker is present but its embedded `pm-fragment-version` is behind `CLAUDE_FRAGMENT.md.template` (a missing line = version 1), the fragment is treated as **stale** and refreshed via a `<file>.new` side-file for manual merge — never overwritten in place
