...
| `project-manager` | extend-in-place | project-manager (P0) | 6-9 | pending | Slug retained. Sole owner of session handoff; gains loop-prompt-composer, backlog-burndown, learnings-ledger, backfill-features. Stages product-inception-pack + enhancement-bundle as `status: draft` stubs (resolution #2). **PM Lifecycle Redesign (2026-07-16, `docs/reports/2026-07-16-pm-lifecycle-redesign.review.md`): added sub-skills `capture`/`groom`/`task`/`retro` (commands `/pm-capture` `/pm-groom` `/pm-task` `/pm-retro`), the `docs/backlog.md` intake store + `scope-manifest.md` chore-lane guard, and STATUS §4 generation. PRs #77–#82.** |
