...
**Author:** Claude (Opus 4.8) · **Date:** 2026-07-16 · **Status:** proposal (react-before-code) · **Rev 5** — closes a TOCTOU gap: chore authorization is now frozen at promotion (`authz_snapshot` + `manifest_sha`) and a chore commit may not edit its own authorization artifacts. Rev 4 added the machine-enforced `scope-manifest.md`. Rev 3 added guard chore-validation, atomic BL-id allocation, `what-next` triaged-only. Rev 2 added the plan-review corrections. *(Adversarial loop converged after 3 passes: existence → boundary → temporal integrity.)*
...
**Authorization is frozen at promotion — the boundary is not re-evaluated against mutable state.** (Adversarial-review Finding, 3rd pass — a time-of-check/time-of-use gap.) Because the guard resolves classification against the *current* backlog row and *current* manifest, a single commit could otherwise weaken the manifest (or flip a BL row to `chore`) **and** carry product-scope changes under the chore lane — a self-authorizing change. Closed two ways:

- **Frozen snapshot.** At promotion, `/groom`/`/task` write an immutable `authz_snapshot` into the chore task's frontmatter: `bl_type`, `bl_status`, `manifest_sha` (sha256 of `scope-manifest.md` at promotion), and the reviewed file scope — reusing the task template's existing **`files_allowed`** globs as that scope. The guard validates staged files ⊆ `files_allowed`, and `files_allowed` ∩ `product_scope` (under the **frozen** manifest) = ∅. If the live manifest's hash no longer matches `manifest_sha`, the task is stale → **FAIL, re-groom to re-freeze**. Classification can never be silently broadened under an in-flight task.
- **Same-commit self-authorization block.** The guard **rejects any commit that relies on a chore task and also edits `docs/workflow/scope-manifest.md` or the `backlog_ref` row's authorization fields** (`type`/`status`). Classification changes must land *separately*, through the normal reviewed-diff path, before a chore task can depend on them.

Accepted tradeoff: a manifest edit invalidates in-flight chore tasks until re-groomed. That is the conservative-correct behaviour — a changed classification *should* force re-review — and re-groom is cheap.
...
**Changed:** `sync-status/SKILL.md` (§4 generated from backlog + homeless issues; +`assignee` in §2/§3; **+`BL-*` uniqueness check across live+archive**), `project-manager/SKILL.md` (pipeline diagram + chore lane + directory conventions + command table), `init-project/SKILL.md` + templates (scaffold backlog files + `scope-manifest.md` default + §4 generated fence), **`guard-pm-flow.ps1.template` (new chore-task validation path: `backlog_ref` resolves + row is chore-class + scope-manifest intersection against the *frozen* `authz_snapshot`/`manifest_sha` — `product_scope` hit fails closed, unclassified needs `scope_confirmed`, missing/changed manifest fails closed, and a commit that edits its own scope-manifest/backlog authorization is rejected)**, `reinit/SKILL.md` (lift a curated §4 into backlog rows on adoption), `update-tasks/SKILL.md` (flip `BL` row to `done`; reconcile merged PR↔task), `tasks/README.md` template (document *where ad-hoc work goes* + the enforced pseudo-CAP contract), `task-file-template.md` (+`assignee`, +`backlog_ref`, +`scope_confirmed`, +`authz_snapshot` frozen at promotion; reuses existing `files_allowed` as the reviewed scope), `what-next/SKILL.md` (read backlog.md, **rank only triaged/executable work; untriaged rows route to `/groom`**), `validate.ps1` (BL-id uniqueness gate).
...
| Chore authorization is mutable at use-time — TOCTOU (3rd pass) | high | **Incorporated (Rev 5)** — authorization frozen at promotion (`authz_snapshot` + `manifest_sha` + `files_allowed` scope); stale-on-manifest-change fails closed; a chore commit that edits its own scope-manifest/backlog authorization is rejected (§2.5). Classification changes must land separately. |
...
4. **`task` express + guard enforcement + `update-tasks` reconciliation** — the guard-honoring fast path and `BL`-row closeout. **Land `docs/workflow/scope-manifest.md` (+ template) and the full `guard-pm-flow.ps1` chore-validation path — `backlog_ref` + chore-class row + scope-manifest intersection — *in this PR, before or with `/task`*,** so the scope boundary is machine-enforced the moment the express lane exists (never shipped ahead of its guard). Register `/task`; write `scope_confirmed` only after the spec-authority re-assert, and only for unclassified files. **Include the frozen `authz_snapshot` (bl type/status + `manifest_sha` + `files_allowed` scope) written at promotion, and the guard's same-commit self-authorization block** — authorization must be immutable and un-self-editable from day one of the lane.
