...
> **Status: complete.** All tasks shipped — T1–T9 plus the two follow-on tasks (T5b,
> T5c) spawned by findings surfaced during execution. PR numbers: T1/T2 → #58, T3 →
> #59, T4 → #61, T5 → #62, T5b → #63, T6 → #65, T5c → #66, T7 → #68, T8 → #70, T9 →
> #71. This file is now a closed execution record; see the [Reference
> map](#reference-map) appendix for which requirements/charter section backs each
> task.
>
> **Frontmatter note:** this file's frontmatter predates
> [`template.md`](./template.md)'s schema (`status`/`failures`/`last_updated`/
> `external_issue`) — it was written before the templated task pipeline existed, so
> the two intentionally don't match; not a drift to reconcile.
...
- Never move, rename, or delete anything under `claude/skills/`, `codex/skills/`, or
  ...
  (requirements [N1/N2](../requirements/canonical-repo.md#non-goals)). Skill
  structure belongs to [`docs/reorg/charter.md`](../reorg/charter.md).
- `install-skills.ps1` stays at repo root (requirements
  [D5](../requirements/canonical-repo.md#resolved-decisions)).
- New PowerShell scripts: `#Requires -Version 7.0`, cross-platform (no Windows-only
  APIs), `utf8NoBOM` output, ordinal sorting, and the requirements [§6 — Script
  requirements](../requirements/canonical-repo.md#script-requirements) shared
...
- **PR 2 → PR 3:** T3's `scripts/README.md` documents conventions
  ([D5](../requirements/canonical-repo.md#resolved-decisions) trust model,
...
- **Work plan:**
  ...
  2. Write `shared/README.md`: the
     [D1](../requirements/canonical-repo.md#resolved-decisions) vendor-neutral test
     (quote it), the four classes,
  ...
  4. Write `shared/workflows/README.md` — must include the requirements [§5 — Target
     structure](../requirements/canonical-repo.md#target-structure) boundary
  ...
  6. Write `shared/plugins/README.md`: pure reference list; every entry carries
     ...
     `external-skill-intake` (charter
     [§4](../reorg/charter.md#4-single-owners)) once that sub-skill ships.
  7. `workflows/`, `configs/`, `plugins/` contain only their README
     ([D7](../requirements/canonical-repo.md#resolved-decisions)).
...
- [x] **Status:** done — PR #58
- **Agent / model:** general-purpose / sonnet
- **Description:** Give every root-level folder a prescriptive README
  ([G4](../requirements/canonical-repo.md#goals)).
- **Work plan:**
  ...
  2. `codex/README.md` — on-demand mirror + source-version stamp (charter
     [§5](../reorg/charter.md#5-cross-cli-mirror-policy)); instructions subtree
     marked "mirror policy: TBD
     ([OQ4](../requirements/canonical-repo.md#open-questions))".
  3. `gemini/README.md` — frozen at current set (charter
     [§5](../reorg/charter.md#5-cross-cli-mirror-policy)); same
     [OQ4](../requirements/canonical-repo.md#open-questions) line for instructions.
  4. `docs/README.md` — map `requirements/`, `plans/`, `reorg/` (binding governance),
     `reports/`.
  5. Untrack `logs/` per [P5](../requirements/canonical-repo.md#proposals): `git rm -r
     --cached logs/`, add `logs/` to `.gitignore`; no `logs/README.md`
     ([G4](../requirements/canonical-repo.md#goals) applies to tracked folders only).
...
- **Acceptance criteria:** READMEs exist for `claude/`, `codex/`, `gemini/`, `docs/`;
  ...
  governs; vendor instruction sections carry the
  [OQ4](../requirements/canonical-repo.md#open-questions) TBD line rather than an
  invented policy; `logs/` absent from `git ls-files` and present in `.gitignore`
  (local file untouched on disk); no README contradicts
  [`docs/reorg/charter.md`](../reorg/charter.md) or the requirements doc.
...
- **Work plan:**
  1. Create `install-to-project.ps1`, `push-to-profile.ps1`, `sync-installed.ps1`,
     ...
     requirements [§6 — Script
     requirements](../requirements/canonical-repo.md#script-requirements) contracts
     **including the shared conventions table** (common
  ...
  3. Write `scripts/README.md`: table of all scripts incl. existing
     ...
     (requirements [D5](../requirements/canonical-repo.md#resolved-decisions))**, and
     the regeneration rule (charter rule 6). Cross-link the
...
- **Acceptance criteria:** 5 stubs + README exist; `Get-Help ./scripts/<stub>.ps1`
  ...
  textually consistent with requirements [§6 — Script
  requirements](../requirements/canonical-repo.md#script-requirements) incl. shared
  conventions; trust-model language present.

### T4 — Implement `audit.ps1` (+ generator flags)
- [x] **Status:** done — PR #61
- **Agent / model:** general-purpose / opus (multi-check logic; single-parser design)
- **Description:** Real, read-only archive health check per requirements [§6 — Script
  requirements](../requirements/canonical-repo.md#script-requirements).
- **Work plan:**
  ...
  2. Implement checks (each → finding record with severity `error|warn|info`) exactly
     as listed in requirements [§6 —
     Script requirements](../requirements/canonical-repo.md#script-requirements):
  ...
  4. Subprocess safety: explicit interpreter + argument array (no shell string
     interpolation), bounded non-predictable temp path (requirements [§6 — Script
     requirements](../requirements/canonical-repo.md#script-requirements) shared
     conventions).
...
- [x] **Status:** done — PR #62
  ([P1](../requirements/canonical-repo.md#proposals) approved — sub-step 3b delivered)
- **Agent / model:** general-purpose / sonnet (script does the sweep; agent writes the script)
- **Description:** Make `category:` frontmatter the single source of truth
  ([D8](../requirements/canonical-repo.md#resolved-decisions)).
- **Work plan:**
  ...
  3. Update `generate-manifest.py`: (a) read `category:` from frontmatter and delete
     ...
     `scripts/README.md` ([P1](../requirements/canonical-repo.md#proposals) —
     approved).
...
- **Acceptance criteria:** every Claude `SKILL.md` has `category:`; generator has no
  ...
  (+ `schemaVersion` if [P1](../requirements/canonical-repo.md#proposals) approved);
  `audit.ps1` exits 0 on the live repo and the seeded-fixture escalation check
  passes; no non-frontmatter content changes under `*/skills/` (`git diff`
  inspection); `scripts/README.md` status current.
...
- **Work plan:**
  ...
  5. README parity check for the 10 renamed skills (charter
     [§6](../reorg/charter.md#6-parity-follow-through-every-deletion-pr) duty).
...
- [x] **Status:** done — PR #65
- **Agent / model:** general-purpose / sonnet
- **Description:** Render the full asset catalog from `manifest.json`
  ([D2](../requirements/canonical-repo.md#resolved-decisions)).
- **Work plan:**
  ...
  3. Content: per-vendor skill tables (name-link, category, description, cross-vendor
     availability), instruction tables **including Codex/Gemini instructions**
     ([OQ4](../requirements/canonical-repo.md#open-questions) — they exist but are
     uncounted today), command tables, per-class shared-asset
...
- **Work plan:**
  1. New README: ≤150 lines AND ≤600 words of body prose (code blocks, tables, link
     lists excluded from the word count) — the mechanical proxy for
     [G1](../requirements/canonical-repo.md#goals)'s 2-minute read. Contents: mission
     statement (requirements [§1 — Mission](../requirements/canonical-repo.md#mission)),
     what-it-is/for-whom/how, tree map (requirements [§5 — Target
     structure](../requirements/canonical-repo.md#target-structure) style, no counts),
     quick start (installer one-liner preserved verbatim +
     [D5](../requirements/canonical-repo.md#resolved-decisions) trust-model note +
     integrity-conscious alternative),
  ...
  3. Project `CLAUDE.md`: replace the directory-tree annotation counts ("15 agent
     ...
     90/90/5 figures are in the user-global CLAUDE.md — out of scope,
     [OQ3](../requirements/canonical-repo.md#open-questions).)
...
### T8 — Local validation gate *([P2](../requirements/canonical-repo.md#proposals) approved as modified: local, not GitHub Actions)*
- [x] **Status:** done — PR #70
- **Agent / model:** general-purpose / sonnet
...
- **Work plan:**
  1. Implement `scripts/validate.ps1`: regenerate `CATALOG.md` from the *committed*
     ...
     `-Json` flag. Honors requirements [§6 — Script
     requirements](../requirements/canonical-repo.md#script-requirements) shared
     conventions.
  2. Implement `scripts/install-hooks.ps1`: opt-in installer that sets repo-local
     ...
     global git-push-opens-Zed PreToolUse hook (charter
     [§7](../reorg/charter.md#7-hook-interactions) precedent) — the git hook
...
### T9 — `CONTRIBUTING.md` *([P4](../requirements/canonical-repo.md#proposals) approved)*
- [x] **Status:** done — PR #71
- **Agent / model:** general-purpose / sonnet
- **Description:** Codify contribution rules
  ([P4](../requirements/canonical-repo.md#proposals) scope).
- **Work plan:**
  1. Sections, in order: (a) add a skill — frontmatter schema incl. `category:`,
     ...
     asset class — README-first rule,
     [D1](../requirements/canonical-repo.md#resolved-decisions) vendor-neutral test;
     (d) governance — reorg
     charter precedence for anything touching skill structure, git workflow digest
     (branch naming, PR limits, `/ship`).
  2. Source material: requirements [§4–§7](../requirements/canonical-repo.md#resolved-decisions)
     (Resolved decisions through Proposals), charter
     [§§2](../reorg/charter.md#2-slug-rules)/[5](../reorg/charter.md#5-cross-cli-mirror-policy)/[6](../reorg/charter.md#6-parity-follow-through-every-deletion-pr),
     `scripts/README.md`, `~/.claude/rules/git-workflow.md`.
...
- **[P3](../requirements/canonical-repo.md#proposals) execution —
  CHANGELOG-per-release: done.** The archive was in the MISSING state of its own
  Release-Automation Standard (no generator, no `.github/`), which is why
  `CHANGELOG.md` sat three releases stale at v0.5.0. Resolved by installing
  ...
  gap cannot silently reopen. Note this does **not** reverse
  [P2](../requirements/canonical-repo.md#proposals): the *validation* gate stays
  local; `release.yml` only publishes a GitHub Release on an already-cut tag.
- **[P3](../requirements/canonical-repo.md#proposals) execution — published SHA256**
  for `install-skills.ps1` remains open; it belongs to the `/release` flow (github
  skill, reorg-governed). Coordinate with the github bundle rather than tasking it
  here; T3's `scripts/README.md` documents the SHA256 verification step for
  consumers once releases carry it.
- Hosted-CI mirror of the T8 local validation gate
  ([P2](../requirements/canonical-repo.md#proposals) as modified).
- [OQ1](../requirements/canonical-repo.md#open-questions) asset-first layout study
  (post reorg iteration 26).
- [OQ2](../requirements/canonical-repo.md#open-questions) Python ports of all
  PowerShell lifecycle scripts (contract = requirements [§6 — Script
  requirements](../requirements/canonical-repo.md#script-requirements) shared
  conventions).
- [OQ3](../requirements/canonical-repo.md#open-questions) fleet-wide stale-count
  sweep (reorg iteration 10 vehicle).
- [OQ4](../requirements/canonical-repo.md#open-questions) instructions
  mirror-policy ruling.
...
Each task's "References" footer has been folded into this single table (info
preserved, repetition cut). `requirements` links point into
[`../requirements/canonical-repo.md`](../requirements/canonical-repo.md);
`charter` links point into [`../reorg/charter.md`](../reorg/charter.md).

| Task | Referenced sections |
|---|---|
| T1 | requirements: [Resolved decisions](../requirements/canonical-repo.md#resolved-decisions) (D1/D6/D7), [Target structure](../requirements/canonical-repo.md#target-structure) (class boundary) · charter: [§4 Single Owners](../reorg/charter.md#4-single-owners) |
| T2 | requirements: [Target structure](../requirements/canonical-repo.md#target-structure) (coverage rule), [Open questions](../requirements/canonical-repo.md#open-questions) (OQ4) · charter: [§5](../reorg/charter.md#5-cross-cli-mirror-policy), [§6](../reorg/charter.md#6-parity-follow-through-every-deletion-pr) · root `CLAUDE.md` |
| T3 | requirements: [Script requirements](../requirements/canonical-repo.md#script-requirements) (incl. shared conventions), [Resolved decisions](../requirements/canonical-repo.md#resolved-decisions) (D3, D5) · `install-skills.ps1` (style precedent only — its 5.1 floor is not to be copied) · `.claude/commands/install-skill.md`, `.claude/commands/audit-skills.md` |
| T4 | requirements: [Script requirements](../requirements/canonical-repo.md#script-requirements) · charter: [§5](../reorg/charter.md#5-cross-cli-mirror-policy), [§6](../reorg/charter.md#6-parity-follow-through-every-deletion-pr), [§10](../reorg/charter.md#10-verified-counts-2026-07-10-audit) · `scripts/generate-manifest.py` (block-scalar frontmatter handling, lines ~129–144) |
| T5 | requirements: [Resolved decisions](../requirements/canonical-repo.md#resolved-decisions) (D8), [Script requirements](../requirements/canonical-repo.md#script-requirements), [Proposals](../requirements/canonical-repo.md#proposals) (P1) · `scripts/generate-manifest.py:14-95` (`CATEGORIES` dict + `STANDARD_SKILLS` list) · pre-rewrite README category tables (git history) |
| T5b | requirements: [Script requirements](../requirements/canonical-repo.md#script-requirements) (`audit.ps1` contract), [Non-goals](../requirements/canonical-repo.md#non-goals) (N1/N2) · charter: [§6](../reorg/charter.md#6-parity-follow-through-every-deletion-pr) (parity follow-through) · T4 (audit implementation) |
| T5c | requirements: [Resolved decisions](../requirements/canonical-repo.md#resolved-decisions) (D8), [Script requirements](../requirements/canonical-repo.md#script-requirements) · charter: [§6](../reorg/charter.md#6-parity-follow-through-every-deletion-pr) (parity follow-through) · T5, T5b (override precedent) · T6 (the CATALOG.md run that surfaced the drift) |
| T6 | requirements: [Resolved decisions](../requirements/canonical-repo.md#resolved-decisions) (D2), [Script requirements](../requirements/canonical-repo.md#script-requirements), [Proposals](../requirements/canonical-repo.md#proposals) (P1) · `manifest.json` · current README tables (content reference only — do not preserve their hand-drift) |
| T7 | requirements: [Mission](../requirements/canonical-repo.md#mission) (§1), [Goals](../requirements/canonical-repo.md#goals) (G1), [Acceptance criteria](../requirements/canonical-repo.md#acceptance-criteria-merged-mission--interview) (items 1 and 5), [Resolved decisions](../requirements/canonical-repo.md#resolved-decisions) (D2, D5) · `CATALOG.md` (from T6) · project `CLAUDE.md` |
| T8 | requirements: [Proposals](../requirements/canonical-repo.md#proposals) (P2, as modified), [Resolved decisions](../requirements/canonical-repo.md#resolved-decisions) (D3), [Script requirements](../requirements/canonical-repo.md#script-requirements) · T4/T6 scripts · charter: [§6](../reorg/charter.md#6-parity-follow-through-every-deletion-pr), [§7](../reorg/charter.md#7-hook-interactions) (hook interactions) |
| T9 | requirements: [Proposals](../requirements/canonical-repo.md#proposals) (P4) · charter: [§2](../reorg/charter.md#2-slug-rules), [§5](../reorg/charter.md#5-cross-cli-mirror-policy), [§6](../reorg/charter.md#6-parity-follow-through-every-deletion-pr) · `~/.claude/rules/git-workflow.md` |
