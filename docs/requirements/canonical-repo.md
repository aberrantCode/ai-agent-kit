...
| `push-to-profile.ps1` | **implemented** | Deploy a bundle to `~/.claude/skills/` (or vendor equivalent), stamping provenance. **Nested** layout (sub-skills stay under the bundle; never loose top-level — `audit.ps1` Check 8); parent + each sub-skill `SKILL.md` stamped `installed-from: ai-agent-kit`, commands relocated to the profile `commands/`, strays and `status: draft` skipped. Honors shared safety conventions (preview default, `-Force` writes with backup-before-overwrite, containment-checked paths). |
