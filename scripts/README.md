...
| `push-to-profile.ps1` | implemented | Deploys a named bundle from `<Vendor>/skills/<Name>` to the vendor profile (`~/.<vendor>` by default; `-TargetDir` overrides the root, `-Vendor` selects claude/codex/gemini). **Nested layout** mirroring the archive: parent `SKILL.md` + `sub-skills/**` + `references/**` + `rules/**` + `diagram.html` under `<root>/skills/<Name>/`, companion commands to `<root>/commands/` — never a loose top-level `skills/<sub>/` (that would shadow the bundle's own sub-skill per `audit.ps1` Check 8 and break the parent's `sub-skills/<sub>` delegation). Every `SKILL.md` (parent + each nested sub-skill) is stamped `installed-from: ai-agent-kit`; commands/references/rules are not. Non-member bundle-root files (e.g. stray working docs) are skipped and reported. `status: draft` bundles are skipped. Preview-only by default; `-Force` writes and backs up any existing bundle + overwritten command files under `<root>/.aak-backups/<Name>-<stamp>/` first (no silent clobber). Paths canonicalized + containment-checked; crafted `-Name` rejected. Console summary + `-Json`. Exit `0` success/preview/draft-skip, `1` validation failure, `2` execution error. |
