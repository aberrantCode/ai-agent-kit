...
| `push-to-profile.ps1` | implemented | Deploys a named bundle from `<Vendor>/skills/<Name>` to the vendor profile (`~/.<vendor>` by default; `-TargetDir` overrides the skills root, `-Vendor` selects claude/codex/gemini). **Nested** layout mirroring the archive: parent `SKILL.md` + `sub-skills/**` + `references/**` + `rules/**` + `scripts/**` under `<skillsRoot>/<Name>/`; companion commands relocated to the sibling `commands/` (the discoverable slash-command location — never nested, which would leave `/<cmd>` unreachable). Sub-skills stay nested (never loose top-level — that shadows the bundle's own sub-skill per `audit.ps1` Check 8 and breaks the parent's `sub-skills/<sub>` delegation; also fixes F-KO-05, the missing-`sub-skills/` bug). Every `SKILL.md` (parent + each nested sub-skill) is stamped `installed-from: ai-agent-kit`; commands/references/rules/scripts are copied verbatim. `status: draft` bundles are skipped. Preview-only by default; `-Force` writes and first backs up any existing bundle to a sibling `<skillsRoot>-backups/<Name>.bak-<stamp>` (outside the scanned skills root, so the backup is never loaded as a duplicate). Paths canonicalized + containment-checked; `-Name` restricted to a bare directory name. Console summary + `-Json`. Exit `0` success/preview/draft-skip, `1` validation failure, `2` execution error. |
...
| `validate.ps1` | implemented (T8) | Local pre-PR validation gate (P2, approved as modified: local, not GitHub Actions). Regenerates `CATALOG.md` from the manifest.json on disk and byte-diffs it against the committed file (restoring the original bytes afterward — never leaves a diff in the working tree), then runs `audit.ps1` as a child process and folds its findings/exit code in. Manifest freshness is never re-checked directly here — that would mean a second, timestamp-naive diff that false-positives on day rollover, so it is delegated entirely to `audit.ps1`'s own timestamp-excluded freshness check. Also asserts that every `v*` tag has a matching `## [<version>]` section in `CHANGELOG.md` — deliberately a presence check, not the byte-diff used for `CATALOG.md`, because `CHANGELOG.md` derives from `HEAD` rather than from a committed input, so byte-equality would be unsatisfiable (every commit, including the one that refreshes it, invalidates it) and the gate would never be green; only the tagged sections are immutable, and the volatile `[Unreleased]` block is ignored. Console summary table + `-Json`. Exit `0` clean, `1` catalog-stale, changelog-missing-a-released-section, and/or audit error-severity findings, `2` execution failure. Measured runtime on the live repo: ~5s (target <60s). |
| `Generate-Changelog.ps1` | implemented — installed verbatim from the standard | Deterministic full rebuild of `CHANGELOG.md` from `git log`: `[Unreleased]` = `latestTag..HEAD`, each version section = `prevTag..tag`, grouped by conventional-commit prefix into Keep-a-Changelog sections. Skips `release:` bump commits, and skips scaffolding commits (unparseable subject **and** a `.gitkeep`-only diff — the file check is gated on the subject failing to parse, so well-formed commits cost no extra `git show`). Pins `[Console]::OutputEncoding` to UTF-8 before invoking git: PowerShell decodes native stdout with that property, so under the OEM codepage an em-dash round-trips into durable mojibake — which is why this only corrupted under `-NoProfile`, i.e. every automated invocation. Installed verbatim from `claude/skills/github/sub-skills/release-init/templates/`; the template is the source of truth, so fixes go there and are copied down (see "Regeneration rule"). |
